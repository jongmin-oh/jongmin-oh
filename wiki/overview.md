...
> 완성도 **80%를 의도된 정지선**으로 두는 것이 그 핵심이다 — 남은 20%는 가설이 틀리면 버려지는 부분이라서. 이건 성향이 아니라 **완성도를 추구하다 시간·방향을 잃어본 대가로 정한 규율**이다 (→ [[reflections/80퍼센트-프로토타입]]).
>
...
| **⭐ 80% 프로토타입 최속 구현** | 실행력(성향) + 서버리스 숙련(기술) → 인프라 인지부하 제거(구조). 레플리 3년 8개월간 0→1 서비스 7건 + 운영 도구 9건, 미경험 영역도 착수→배포 (→ [[reflections/80퍼센트-프로토타입]]) |
| **⭐ 비용을 줄여 런웨이를 번다** | 층위별 비용 절감 11건 — 로그 파이프라인(ES→Firehose)·호출 빈도 1/4·컨텍스트 캐싱·과금 모델 전환·WebP·Billing 상시 모니터링. 절감을 '원'이 아니라 **검증 기회**로 환산 (→ [[reflections/비용을-줄이는-사람]]) |
...
- 비용 최적화 11건(층위별) — 건당 30%↓(프롬프트 캐싱) · SaaS 대비 5배↓(GPU 직접 계약) · 50%↓(Batch API) · 장기기억 호출 75%↓ ※ 나머지 절감액은 `[미기록]`
