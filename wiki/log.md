...
## [2026-09-09] ingest | 커리어 중심 명제 — 코더에서 문제해결사로 (본인 구술)

본인 구술: "요구사항을 코드로 옮기는 코더와, 그 윗단의 문제를 근본적으로 이해하고 해결하는 개발자는 다르다. 후자만이 진짜 개발자다. 나는 여태까지 코더였지만 스타트업에서 시행착오를 겪으며 problem solver가 되려 노력했고, 앞으로도 그 방향을 추구한다."

**기존 상태 확인**: 이 명제는 위키에 조각으로만 흩어져 있었다 — overview 헤드라인의 "문제에서 출발하는 AI 엔지니어", 초기-스타트업-시행착오의 루트 진단 solution-first, 개발을-잘한다는-것의 "구현 능력은 상품화됐다". **명제 자체를 정면으로 세운 페이지는 없었음** → 신규 생성 + 위키 최상위 우선순위로 승격.

- 신규: `reflections/코더에서-문제해결사로.md` — 명제(코더 vs 문제해결사 5축 대조), 본인 궤적 4단계(코더 → 균열 → 언어 획득 → 실천), **증거 6건**(RAG 폐기 / 선톡 지표 정의 / A/B 실험 / 유리한 숫자의 자기 기각 / 운영 직접 담당 / Nightly) ※ 같은 날 후속 업데이트로 '판단할 시간을 구조로 확보(서버리스)'가 6번으로 삽입되어 총 7건, 자소서·면접 활용법(3문장 구조 + 원칙 4가지 + 피할 것).
- 승격: index.md의 **핵심 섹션 최상단**(overview·timeline보다 위) + Reflections 최상단에 ⭐ 배치.
- overview.md: 상단 인용부 맨 앞에 **중심 명제** 블록 신설, 핵심 특징 표 첫 행에 ⭐ 행 추가, 관련 페이지 최우선 링크.
- 역링크: 초기-스타트업-시행착오(→ 상위 명제로 명시) · 개발을-잘한다는-것(→ AI 시대 번역판) · 그로스해킹과-제품관점의-전환(→ 전환 3단계 '언어 획득').

서사 위계가 정리됐다: **코더에서 문제해결사로**(명제) ← 시행착오(기원) · 그로스해킹(언어) · 개발을 잘한다는 것(AI 시대 번역) · 프로젝트들(증거).

## [2026-09-09] update | 서버리스를 택하는 '진짜 이유' 2가지 (본인 구술, 아정당 자소서 작업 중)

기존 원칙 3은 서버리스를 **"운영 부담·고정 비용 최소화 → 빠른 검증"** 까지만 적어놨다. 본인 구술로 그 아래 층의 논리 2개가 드러나 보강한다. 둘 다 비용 절감이 아니라 **판단의 문제**다.

- **(1) 고정비 = 리스크**: 가장 경계할 위험은 *검증되지 않은 가설에 고정비를 묶어두는 것*. 고정비는 매몰 비용이자 심리적 족쇄여서 "틀렸으니 접자"는 판단 자체를 막는다. 사용량 비례 과금은 **틀렸을 때의 비용을 0에 수렴**시킨다 → 서버리스는 싸게 만드는 기술이 아니라 **틀릴 자유를 확보하는 설계**. ※ 원칙 4의 '비용 선형 증가 차단'과 축이 다름 — 원칙 4는 *성공(스케일)* 시 비용 곡선, 이건 *실패(검증)* 시 손실 크기.
- **(2) 아낀 공수의 재배치**: 인프라 공수를 줄이면 그 시간이 **고객 문제를 정의하는 데로 재배치**된다. 즉 서버리스는 목적이 아니라 [[reflections/코더에서-문제해결사로]] 명제의 **실행 수단**. 만드는 데 인지 자원을 다 쓰면 구조상 무엇을 만들지 판단할 여력이 안 남는다 → 원칙 6(SDD)과 같은 뿌리(SDD는 판단의 *방법*, 서버리스는 판단할 *시간 확보*).

갱신: skills/working-style.md(원칙 3 제목을 '검증 단계의 낭비를 최소화한다'로 변경 + '왜 서버리스인가 — 두 개의 진짜 이유' 절 신설, 흔한 이유 vs 본인의 이유 대조표) · reflections/코더에서-문제해결사로.md(증거 6번 '판단할 시간을 구조로 확보했다' 신설, 기존 6→7 / 자소서 원칙에 "기술 선호가 아니라 판단의 문제로 말한다" 추가) · overview.md(핵심 특징 ⭐행 신설) · index.md 요약.

※ JD 문구('만드는 데 드는 시간보다 무엇을 만들지 판단하는 데 시간을 쓴다')는 아정당 공고 고유 표현이라 위키에는 넣지 않고 논리만 일반화해 기록.

## [2026-09-12] ingest | 비용 절감 — 본인이 꼽는 최대 강점 (본인 구술)

본인 구술: "레플리에서 가장 잘 했던 것은 비용을 줄이는 일이었다. 절약 정신이 배어 있는 개인 성격이 반영된 것 같다. 돈을 벌어오는 서비스를 획기적인 아이디어로 만든 적은 없지만, 뭐든지 비용에 대한 집착으로 어떻게든 최소한의 비용으로 가능하도록 했다."

신규 6건: ①ES 로그 수집 → AWS Kinesis Firehose 전환 ②장기기억 호출 채팅 1회당 1회 → 4회당 1회(호출 75%↓) ③Gemini 컨텍스트 캐싱 ④저호출 서비스 온디맨드 → Lambda/SAM 서버리스 전환 ⑤AWS Billing 정기 모니터링 ⑥이미지 전량 WebP 전환(CDN 전송량↓).

**기존 상태 확인**: 비용 관련 기록은 위키 전역에 흩어져 있었다 — 프롬프트 캐싱 30%↓(캐릭터챗봇API), GPU 직접 계약 5배↓·번역 캐싱(이미지생성서비스), Batch API 50%↓(정밀의료), RAG 폐기(장기기억), working-style 원칙 3·4. **강점 자체를 정면으로 세운 페이지는 없었음** → 신규 생성. 신규 6 + 기존 5 = **총 11건**으로 묶임.

- 신규: `reflections/비용을-줄이는-사람.md` — 명제(절감 = 런웨이 = 검증 기회, 단위는 '원'이 아니라 '시도 횟수'), 증거 6층위 표(저장·호출빈도·프로바이더 기능·과금모델·관측·전송량) + 기존 5건, 강점 근거 3가지(마진 100% 현금 / 층위를 안다는 건 아키텍처 이해도 / 아무도 시키지 않는 일 = 오너십), **세 축 구분표**(원칙 3=실패 시 손실 · 원칙 4=성공 시 곡선 · 이 페이지=상시 청구서), 그림자 2가지, 자소서 활용법.
- **핵심 해석**: 6건이 서로 다른 층위라는 점이 임팩트의 근거다. 한 번의 큰 절감이 아니라 재현되는 습관이라는 증거이며, 비용 구조를 알려면 시스템 전체를 알아야 하므로 아키텍처 이해도의 간접 증명이기도 하다. 또한 AWS Billing 모니터링(⑤)이 나머지를 *발견한 방법*이므로 서사에서 먼저 배치해야 한다.
- **그림자 기록**: (a) 절감 목적화의 위험 — 선톡에서 "비용 부족 → 싼 모델"로 직행, 배치 추론(같은 비용·상위 모델)이라는 축을 놓침. 올바른 질문은 "얼마나 싸게"가 아니라 "같은 돈으로 얼마나 더". (b) 본인 인정대로 **수익 축은 미증명** — 강점이 비대칭이라는 사실을 지우지 않고 표시.
- **남은 과제**: 6건 모두 절감 금액·비율 `[미기록]`. 자소서에서 이 강점은 숫자 없이는 절반만 전달됨 → 전/후 월 청구액 또는 %, 특히 **전체 월 인프라 비용 추이** 복원 필요.
- 갱신: index.md(Reflections에 ⭐ 행 + skills 요약) · overview.md(핵심 특징 ⭐행 + 주요 지표 1행) · skills/working-style.md(원칙 4에 '설계 시점 vs 상시 운영' 구분 + 링크) · skills/technical.md(Kinesis Firehose·AWS Billing 모니터링·WebP 전환 / Gemini 컨텍스트 캐싱 행 추가) · entities/companies/레플리.md(개발 철학에 1행).

## [2026-09-12] ingest | 80% 프로토타입 — 본인이 꼽는 강점 ② (본인 구술)

본인 구술: "80% 완성도까지 프로토타입을 아주 빠르게 개발한다. 배경은 실행력·추진력 + 서버리스 아키텍처를 잘 다뤄서 인프라에 신경을 덜 쓰고 프로덕트에 집중할 수 있었던 것. 그래서 스타트업에서 아주 많은 서비스를 빠르게 개발할 수 있었다. 즉 빠른 PoC 제품을 만들 수 있는 장점."

**기존 상태 확인**: overview 헤드라인 강점("초기 검증 제품 최속 구현")과 working-style 원칙 3에 한 줄씩만 있었고 **독립 페이지는 없었음**. 이번 구술로 (a) "80%"라는 구체적 기준과 (b) 속도의 출처 3층이 드러나 신규 생성.

- 신규: `reflections/80퍼센트-프로토타입.md`
- **핵심 재프레이밍**: 80%는 능력의 한계선이 아니라 **의도된 정지선**이다. 남는 20%(엣지케이스·최적화·운영 완성도)는 *가설이 틀리면 통째로 버려지는 20%*. 끝나는 지점은 "흠이 없을 때"가 아니라 **"가설의 참·거짓이 갈릴 때"**. → 그냥 "80%"라고 말하면 미완성 변명, 정지선으로 말하면 완성도 판단력. 서술 순서가 결정적.
- **속도의 출처 3층 분해**: ①성향(실행력·추진력) ②기술(Lambda/SAM 숙련 → 인프라가 병목이 아님) ③**구조(인지 자원이 인프라가 아니라 프로덕트로 감)**. 방어 가능한 핵심은 ③ — 손이 빠른 건 개인기라 재현 불가, 병목의 구조적 제거는 방법론이라 이식 가능.
- 증거: 레플리 0→1 서비스 7건 + 운영 도구 9건 + 개인 6건. 속도 사례 — AI소설 2개월 단독 완주 / 이미지 생성(미경험 영역 완전 0→1) / 맛보기 웹(React 미경험 → AI 코딩툴로 출시) / Nightly(미경험 스택 → 람다 배포). 패턴: **미경험이 착수를 막지 않는다**.
- **그림자 3**: (a) 80%가 정지선이 아니라 한계선이 되는 순간 = 기술부채 → 정당성 조건은 **재개 조건**의 존재(장기기억 Redis → 한계가 데이터로 보이자 DynamoDB, 이게 작동 사례). (b) **만드는 비용이 싸면 안 만들 결정이 더 어려워진다** — 시행착오 5번('만들기가 만병통치약')과 정면 긴장. 그래서 이 강점은 [[reflections/코더에서-문제해결사로]]와 **짝으로만 안전**. (c) 속도는 산출량이지 성과가 아니다 — 빠르게 접은 사례(RAG 폐기)를 같이 말해야 판단력의 증거가 된다.
- **강점 지형 정리**: 실행력+서버리스 → 인지부하 제거 → (시간 낭비 제거 = 이 페이지) + (돈 낭비 제거 = [[reflections/비용을-줄이는-사람]]) → 공통 목적은 **시도 횟수 증가**. 두 강점이 한 뿌리임이 드러남.
- **남은 과제**: 착수→출시 기간이 AI소설(2개월) 외에 전부 `[기간 미기록]`. "빠르다"는 비교 기준 없이는 전달 안 됨. 동시 병행 개수도 복원 가치 높음.
- 갱신: index.md(Reflections ⭐ 행) · overview.md(헤드라인 강점에 80% 정지선 문장 + 핵심 특징 행을 ⭐로 승격) · skills/working-style.md(원칙 3 = 수단, 이 페이지 = 결과) · reflections/비용을-줄이는-사람.md·코더에서-문제해결사로.md·초기-스타트업-시행착오.md 역링크.

## [2026-09-12] update | 80% 정지선의 기원 — 완성도 추구의 실제 손실 3종 (본인 구술, 같은 날 후속)

본인 구술: "완성도를 추구하다가 시간만 날리고, 불필요했거나 방향이 잘못된 경우가 많았기 때문이다. 이건 내가 스타트업에서 느꼈던 경험이다."

80% 정지선이 **이론이 아니라 경험의 산물**임이 드러나 `reflections/80퍼센트-프로토타입.md`에 '왜 하필 80%인가 — 이건 이론이 아니라 데인 자리다' 절 신설.

- **손실 3종 분류**(각각 [[reflections/초기-스타트업-시행착오]] 클러스터 A와 대응): ①시간만 날림(←2번, 사용자가 먼저 이탈) ②애초에 불필요(←1번, 완벽하게 숨겨 만들었더니 수요 없음) ③방향 오류(←3번, nice to have가 스코프를 키움).
- **핵심 비대칭**: 0→80%의 공수는 최소한 "틀렸다"는 정보를 남기지만, **80→100의 공수는 가설이 깨지면 정보도 결과물도 없이 전액 손실**이다. 이게 정지선을 80에 둔 정량적 근거.
- **인식 (1) 완성도 ↔ 피드백 시점 트레이드오프**: 완성도를 높일수록 오답을 더 늦게·더 비싸게 안다. 80%에서 멈추는 건 게으름이 아니라 **오답을 빨리 발견하기 위한 설계**.
- **인식 (2) 공수도 매몰비용, 돈보다 질기다**: working-style 원칙 3의 '고정비 = 심리적 족쇄' 논리를 **개발 공수**로 확장. 오래 다듬을수록 애착이 붙어 방향 전환이 어려워지고, 돈과 달리 공수는 내 시간·자존심이라 더 안 놓아진다 → 20%를 남기는 건 **방향 전환의 심리적 비용을 미리 낮추는 일**.
- **왜 하필 '숫자'인가**: 시행착오 3번의 메타 깨달음("알면서도 또 그랬다")에 대한 대응. 알면서 못 멈추면 그때그때 판단에 맡길 수 없다 → **80%는 측정치가 아니라 사전 약속**. 규율은 만들기 *전에* 정해둘 때만 작동하고, 만드는 중에는 항상 "이것만 더"가 이긴다.
- 자소서 원칙 추가: **기원을 붙일 것** — "성향이 아니라 손해를 보고 정한 규율입니다"가 있어야 방법론 흉내가 아닌 수업료로 들린다.
- 갱신: 80퍼센트-프로토타입.md(신설 절 + 명제 표에 '오답을 아는 시점' 행 + 자소서 원칙) · 초기-스타트업-시행착오.md(역링크를 '클러스터 A가 강점으로 전환된 자리'로 상세화) · overview.md 헤드라인 · index.md 요약.

## [2026-09-13] ingest | 이미지 생성 서비스 GPU 하드웨어 장애 2회 (본인 구술)

**사실 정정**: 가비아 GPU는 클라우드 서비스가 아니라 **하드웨어(GPU) 대여 + 네트워크 서비스**. 기존 페이지의 "시간당 단가 계약"은 맞지만 성격이 클라우드처럼 읽혔음 → 명시.

장애 2회, 둘 다 **하드웨어가 GPU를 인식하지 못하는** 증상.
- 1차: 드라이버 자동 업데이트로 버전 불일치. 임시 대응 = AWS EC2 g6 기동 + 라우팅 전환, 근본 조치 = 드라이버 버전 고정 + 자동 업데이트 off 후 원복.
- 2차: 실제 장비 결함. 벤더 회수·수리 필요 + **주말 미운영인데 서비스 피크는 주말**. AWS 단가가 비싸서 RunPod A40($0.5/h)로 대체, 수리 완료까지 유지. 이후 컨테이너 이미지를 Docker Hub에 상시 보관 → 재발 시 docker 명령만으로 기동.

- **핵심 해석 1**: 2차의 병목은 GPU가 아니라 **복구 시점의 통제권**이었다. 저렴한 시간당 단가에 복구 시간은 포함되어 있지 않다 — 5배 절감의 대가가 벤더 일정 종속. → [[reflections/비용을-줄이는-사람]] 그림자와 같은 계열(싼 선택의 비가시 비용).
- **핵심 해석 2**: 해답이 "비싼 계약으로 회귀"가 아니라 **핫스왑 경로 상비**였다는 점. 싼 계약을 유지하면서 리스크만 분리했다.
- **핵심 해석 3**: 서빙 엔진을 직접 만들어 컨테이너로 포장해둔 것([[projects/personal/ComfyUI-API-서버]])이 공급자 교체를 가능하게 했다. 자체 구현의 *의도하지 않은* 두 번째 이득 — 락인 회피.
